...
import {
  ...
  FiUser, FiUsers, FiX, FiArrowLeft, FiCircle, FiLayers, FiFileText
} from 'react-icons/fi';
...
const iconMap = {
  ...
  publication: <FiFileText />, contact: <FiMail />, system: <FiCpu />, terminal: <FiTerminal />
};
...
function App() {
  ...
  const launch = (app) => {
    ...
    setRecentApps(current => [app, ...current.filter(item => item.id !== app.id)]);
    setFocused(app.id); setStartOpen(false); setSearchOpen(false); setMobileRecents(false); setShowWelcome(false);
  };
  ...
  return <div className="os-shell">
    <main className={`desktop-surface${mobileRecents ? ' mobile-recents-mode' : ''}`} onClick={() => { setStartOpen(false); setSearchOpen(false); }}>
      ...
      {mobileRecents && <section className="mobile-recents-panel" aria-label="Recent apps"><span className="eyebrow">RECENT APPS</span>{recentApps.length ? recentApps.map(app => <button key={app.id} onClick={() => launch(app)}><span className="desktop-icon-image">{iconMap[app.id]}</span><strong>{app.label}</strong></button>) : <p className="muted">No recent apps</p>}</section>}
      {showWelcome && <section className="desktop-welcome" aria-label="Portfolio overview"><div className="welcome-copy"><span className="eyebrow">WELCOME BACK</span><h1>{profile.name}</h1><p>{profile.title} building thoughtful systems across AI, software, and design.</p><div className="welcome-actions"><button onClick={() => launch(apps[0])}><FiUser /> Open profile</button><button className="secondary" onClick={() => launch(apps[5])}><FiFolder /> View projects</button></div></div><div className="welcome-metrics"><div><strong>{certifications.length}</strong><span>certifications</span></div><div><strong>{skills.length}</strong><span>skills</span></div><div><strong>{experience.length}</strong><span>roles</span></div></div></section>}
      <div className="desktop-icons-container">{desktopApps.map(app => <button className="desktop-icon" key={app.id} aria-label={`Open ${app.label}`} onDoubleClick={() => launch(app)} onClick={event => { event.stopPropagation(); if (window.innerWidth < 768) launch(app); }}><span className="desktop-icon-image">{iconMap[app.id]}</span><span>{app.label}</span></button>)}</div>
      <AnimatePresence>{!mobileRecents && openApps.filter(app => !app.minimized).map((app, index) => <DetailView key={app.id} isOpen onClose={() => close(app.id)} onMinimize={() => minimize(app.id)} title={app.title} icon={iconMap[app.id]} initialX={90 + index * 28} initialY={55 + index * 24} isFocused={focused === app.id} onFocus={() => { setFocused(app.id); setStartOpen(false); }}><div className="window-content">{renderContent(app.id)}</div></DetailView>)}</AnimatePresence>
    ...
    <nav className="mobile-nav" aria-label="Android navigation"><button type="button" aria-label="Recents" onClick={() => { setMobileRecents(true); setStartOpen(false); setSearchOpen(false); }}><FiLayers /></button><button type="button" aria-label="Home" onClick={() => { setOpenApps(current => current.map(app => ({ ...app, minimized: true }))); setFocused(null); setStartOpen(false); setSearchOpen(false); setMobileRecents(false); setShowWelcome(true); }}><FiCircle /></button><button type="button" aria-label="Back" onClick={() => { if (focused) close(focused); setMobileRecents(false); }}><FiArrowLeft /></button></nav>
  </div>;
}
